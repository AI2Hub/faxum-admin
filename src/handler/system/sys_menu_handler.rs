...
pub async fn add_sys_menu(Json(req): Json<AddMenuReq>) -> impl IntoResponse {
    ...
    let add_sys_menu_param = AddSysMenu {, //主键
    ...
    match &mut RB.clone().get() {
        Ok(conn) => {
            let result = diesel::insert_into(sys_menu::table()).values(add_sys_menu_param).execute(conn);
            match result {
...
pub async fn delete_sys_menu(Json(req): Json<DeleteMenuReq>) -> impl IntoResponse {
    ...
    match &mut RB.clone().get() {
        Ok(conn) => {
            let result = diesel::delete(sys_menu).filter(id.eq_any(&req.ids)).execute(conn);
            match result {
                Ok(_u) => BaseResponse::<String>::ok_result(),
                Err(err) => BaseResponse::<String>::err_result_msg(err.to_string()),
            }
...
pub async fn update_sys_menu(Json(req): Json<UpdateMenuReq>) -> impl IntoResponse {
    ...
    match &mut RB.clone().get() {
        Ok(conn) => {
            let result = diesel::update(sys_menu).filter(id.eq(&req.id)).set(update_sys_menu_param).execute(conn);
            match result {
                Ok(_u) => BaseResponse::<String>::ok_result(),
                Err(err) => BaseResponse::<String>::err_result_msg(err.to_string()),
            }
...
pub async fn update_sys_menu_status(Json(req): Json<UpdateMenuStatusReq>) -> impl IntoResponse {
    ...
    match &mut RB.clone().get() {
        Ok(conn) => {
            let result = diesel::update(sys_menu).filter(id.eq_any(&req.ids)).set(status.eq(req.status)).execute(conn);
            match result {
                Ok(_u) => BaseResponse::<String>::ok_result(),
                Err(err) => BaseResponse::<String>::err_result_msg(err.to_string()),
            }
 ...
 *date：2024/12/19 14:21:03
 */
pub async fn query_sys_menu_detail(Json(req): Json<QueryMenuDetailReq>) -> Result<impl IntoResponse, impl IntoResponse> {
    log::info!("query sys_menu_detail params: {:?}", &req);

    match &mut RB.clone().get() {
        Ok(conn) => {
            let sys_menu_sql = sql_query("SELECT * FROM sys_menu WHERE id = ?");
            let result = sys_menu_sql.bind::<Bigint, _>(&req.id).get_result::<SysMenu>(conn);
            match result {
                Ok(x) => {
